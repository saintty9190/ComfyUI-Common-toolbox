...
def _find_cache_entry(nid: str) -> tuple:
    if not nid:
        return ({}, "")
    nid_str = str(nid)
    # 0. 归一化: '6134:1' → '6134', '5580.0.0.3.0.0.6134:1' → '6134'
    base_nid = _chooser_base_id(nid_str) or nid_str
    # 1. 直接精确命中 (原始 id 或归一化 id)
    ...
    # 2. 扫表: 用归一化 nid 匹配 (例如 '5580.0.0.3.0.0.6135' 结尾的 '6135')
    match_nid = base_nid
    for stored_key in list(LATEST_REFS_BY_NODE_ID.keys()):
        ...
        # 2b. 末段数字 == nid (例如 'prefix.6108' 中的 '6108')
...
CHOOSER_SESSIONS: Dict[str, Dict] = {}        # node_key → {"selected","cancelled","ts"}
CHOOSER_LAST_SELECTION: Dict[str, int] = {}   # node_key → 上次选择 (Once Pause 模式复用)
...
def _rerun_generator_inline(gen_node_id, reference_image, chooser_node_key=None,
    ...
    # ── 定位 Generator 缓存 (gen_node_id 为空时自动定位最新) ──
    if not gen_node_id:
    ...
    # composition_modes: 优先用前端传的新值 (Selector 新 toggle 即时生效),
    # 未提供则回退到缓存快照 (与 Generator 上次执行一致)
    ...
    try:
        ...
        result_dict = gen_instance.generate(
            ...
            unique_id=gen_node_id,   # 让 generate() 内部用此 key 写缓存
            prompt_id=None,
        )
    ...
    # generate() 返回 {"ui": {...}, "result": (crops, manifest, bboxes, labels, best)}
    ...
    # ── 双写缓存到 Chooser node_key (前端 _waitRegenRefs 轮询用) ──
    # 前端轮询 refs/${chooser_id}, 但 Generator 缓存 key 是 gen_id.
    # 不双写则 For Loop 场景 (前端链接损坏, 1s POLL 兜底失效) 下前端
    # 永远等不到 ts 变化 → 缩略图不刷新.
    if chooser_node_key:
...
def _chooser_base_id(k) -> Optional[str]:
    ...
    if not base:
        return None
    if ":" in base:                       # 先剥离 GraphBuilder 虚拟后缀
        base = base.split(":", 1)[0]
    if "." in base:                       # 秋叶复合 id 取末段
...
def _find_chooser_session_key(nid) -> Optional[str]:
    ...
    if not s:
        return None
    # 1. 精确命中 (常规: 后端原生 key 或注册过的 alias)
    if s in CHOOSER_SESSIONS:
        return s
    # 2. base-id 等价 (秋叶复合 / GraphBuilder 虚拟 / 两者叠加)
...
LATEST_REFIMG_BY_NODE_ID: Dict[str, Dict] = {}   # chooser node_key → {"ref": {filename, subfolder, type}, "ts"}
...
def _wait_for_choice(node_key: str, total_count: int,
    ...
    try:
        ...
        while _time_module.time() < deadline:
            ...
            # ── 检测重新生成请求 (干净版 Q3b: 同步重跑, 不提交新 prompt) ──
            # 前端 POST regen → session 标记 regen_requested (可选带 regen_modes)
            # → 抛 _ChooserRegen(modes), choose() 捕获后在当前线程内重跑 Generator.
            # seed 用时间戳随机; modes 优先用前端传的新值, 否则取缓存快照.
...
def _register_comp_crop_routes():
    ...
    if _ps_instance is None:
        # ComfyUI 启动早期可能 PromptServer 尚未创建实例, 跳过.
        # 实际场景下: nodes.py 通常在 PromptServer 初始化后才被 import, 此分支为兜底.
    ...
    try:
        ...
        async def _comp_crop_chooser_message(request):
            ...
            if action == "cancel":
                ...
                # 重新生成: 不提交新 prompt, Chooser 内部同步重跑 Generator.
                # 前端可选传 composition_modes (当前 Selector 的 modes_csv),
                # 若提供则优先用新 modes 重跑, 使 Selector 新 toggle 即时生效.
...
# ═══════════════════════════════════════════════════════════════
#  Node 1: CropModeSelector (独立节点, toggle 多选)
# ═══════════════════════════════════════════════════════════════
    ...
    def select(self, modes_csv="lifestyle"):
        ...
        for m in str(modes_csv or "").split(","):
            ...
            if m in valid and m not in picked:
                picked.append(m)
        if not picked:
            picked = ["lifestyle"]   # 全空/无效值兜底
        return (",".join(picked),)


# ═══════════════════════════════════════════════════════════════
#  Node 2: CropGenerator (主节点)
# ═══════════════════════════════════════════════════════════════
    ...
    def generate(self, image, composition_modes, auto_mask_method,
        ...
        if img_np.ndim == 3 and img_np.shape[2] == 4:
            ...
            if int(_alpha.min()) < 255:
                # 有半透明像素: 白底 alpha 合成, 避免直接丢弃 alpha 出现黑边/脏色
        ...
        # ── 1. 主体分析 (mask 未连接时按 auto_mask_method 自动估算) ──
        ...
        # ── 9a. crops batch 输出: 全部候选 resize 到统一尺寸 (无黑边) ──
        #   ComfyUI IMAGE tensor 协议要求 batch 内同尺寸.
        #   这里 resize 到第一张裁切的尺寸 (保持 batch 规整, 喂给 Chooser 显示).
        #   想拿干净原始尺寸, 用 CropChooser (有 reference_image 时按 bbox 重切).
        if len(crops_np) > 0:
            ...
            for i, c in enumerate(crops_np):
                if c.shape[0] != target_h or c.shape[1] != target_w:
                    if _HAS_CV2:
                        ...
                        # Fallback: nearest-neighbor (无 cv2 时)
                        if _HAS_PIL:
                            resized = np.array(
        ...
        try:
            ...
            # 注: 不写全局 latest manifest 文件 — 避免多实例/多插件冲突.
            #     前端通过 history.outputs[genId].images (由 ui.images 触发注册) 获取.
        except Exception as _e:
            crops_manifest = "[]"
            image_refs = []
            print(
        ...
        return {
            "ui": {
                "images": [],                      # 关闭 Generator 节点预览
                "_comp_image_refs": image_refs,    # 自定义 key, 透传到 history + executed
            },
...
# ═══════════════════════════════════════════════════════════════
#  Node 3: Contact Sheet Builder (不变)
# ═══════════════════════════════════════════════════════════════
...
# �══════════════════════════════════════════════════════════════
#  Node 4: CropChooser (Image Chooser 风格)
# ═══════════════════════════════════════════════════════════════
...
class CropChooser:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                # forceInput: 强制渲染为 input socket (非 widget),
                # 这样 Generator 的 crops_manifest 输出能直接连线.
                # 经工作流执行后, 值会写入 widgets_values 数组, 前端可读取.
    ...
    def choose(self, crops, crop_bboxes, mode, preview_rescale,
        ...
        # 解析 crop_bboxes JSON — 提前到等待之前 (Never Pause 需要 best_indices;
        # 后续输出路径也复用同一份 bbox_list)
        bbox_list = []
        ...
        if selected_index == -1:
            while True:
                try:
                    ...
                    # ── 重新生成: 干净版 Q3b 同步重跑 Generator (不提交新 prompt) ──
                    # seed 时间戳随机; modes 优先用前端传的新值 (_regen_exc.modes),
                    # 否则回退缓存快照; gen 自动定位.
                    try:
                    ...
                    if crop_bboxes and str(crop_bboxes).strip():
                        try:
                            ...
                            bbox_list = []
                    # Once Pause: 重跑后清除旧选择, 避免复用旧 index 指向新图
                    _clear_last_selection(node_key)
                    # 继续循环 → 重新调用 _wait_for_choice
            # 等待成功 — 记住选择 (Once Pause 下次复用)
        ...
        if selected_index == -2:
            if reference_image is not None:
                ...
                return (out_ref,)
            # 防御: reference_image 未连线却收到 -2 → 回退 #0
            print(
        ...
        # 优先: 用 reference_image + bbox 切原图 (保留原始分辨率)
        ref_np = None
        H_ref = W_ref = None
        if reference_image is not None:
        ...
        if crops_manifest:
            try:
                ...
                pass  # 静默降级到 batch 兜底 (统一尺寸)

        # 兜底: 直接从 batch 取一张 (batch 已被 Generator resize 到统一尺寸)
